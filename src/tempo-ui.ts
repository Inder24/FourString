...
export class TempoPanel {
  ...
  private render(): void {
    ...
    this.element('#tempo-status').textContent = this.playing ? `${this.schedule.bpm} BPM · ${this.sound === 'drum' ? 'drum pulse' : 'metronome clicks'}` : 'Set a tempo, then press Start beat.';
    for (const sound of ['drum', 'click'] as const) {
